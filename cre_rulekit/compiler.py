...
from .compilers.sqlserver import SqlServerCompiler
...
class RuleCompiler:
    ...
    def to_sql_server_sql(self, rule_json, mid_value=None, select_clause="*"):
        plan = self.plan(rule_json)
        compiler = SqlServerCompiler(self.namespace_config, rule_resolver=self.rule_resolver)
        return compiler.compile(plan, mid_value=mid_value, select_clause=select_clause)
